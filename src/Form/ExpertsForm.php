...
This text will be inserted into the body of questions when they are asked. This helps make editing easier'),
  ...
  if (count($vocabs) == 1) {
    ...
    $voc_list = array();
    $def_vid = 0;  // $faq_ask_settings->get('vocabularies', 0)?
  ...
  // Get All users Having the Role of Answer s
  ...
  if (empty($role_list)) {
    drupal_set_message($this->t('No roles with "answer question" permission were found; only @admin is currently eligible to be an expert. You may want to go to the <a href="@access">Permissions page</a> to update your permissions.',
        array('@access' => Url::fromUserInput('/admin/user/permissions')->toString(), '@admin' => $admin),
        array('langcode' => 'en')),
      'error');
  }
...
//  $result = db_query('SELECT td.tid, td.name, td.description FROM {taxonomy_term_data} td WHERE td.vid IN (' . db_placeholders($vocabs_array) . ') ORDER BY td.weight ASC, td.name ASC', $vocabs_array);
  ...
  if (!empty($rids)) {
    if (in_array(DRUPAL_AUTHENTICATED_RID, $rids)) {
      ...
      $user = \Drupal\user\Entity\User::loadMultiple();
